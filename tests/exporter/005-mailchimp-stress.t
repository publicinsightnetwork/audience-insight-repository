...
my $N_TEST_USERS = $ENV{MAILCHIMP_TEST_SIZE} || 100;
my @TEST_EMAILS = map {"pin5$_.$$\@pinsight.org"} 1 .. $N_TEST_USERS;
my @LIVE_EMAILS = ();
my %seen        = ();
...
for my $e (@TEST_EMAILS) {
    push @sources, create_source($e);
}
pass('setup - sources created');

diag("subscribe all new sources");
...
is( scalar @{ $test_bin->sources },
    $N_TEST_USERS, "setup - test bin_source count" );
...
is( $count, $N_TEST_USERS, "test export - exportable_emails count" );
is( scalar( keys %uniq ),
    $N_TEST_USERS, "test export - exportable_emails unique" );
...
is( $test_exp->completed, ( $N_TEST_USERS - 5 ), "test export - completed" );
