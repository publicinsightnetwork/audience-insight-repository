...
publicresp2xml.pl - convert db records to XML for indexing

=head1 SYNOPSIS

 publicresp2xml.pl [opts] [srs_idN .. srs_idNN]
    --help
...
unless ($quiet) {
    AIR2::Utils::logger(
        "Determining which PublicSrcResponseSet records to serialize...\n");
}

my $pks = AIR2::SearchUtils::get_pks_to_index(
    lock_file     => $lock_file,
    class         => 'AIR2::PublicSrcResponseSet',
    column        => $column_name,
    sql_all       => AIR2::PublicSrcResponseSet->get_sql_all(),
    sql_all_count => AIR2::PublicSrcResponseSet->get_sql_all_count(),
    mod_since     => $mod_since,
    quiet         => $quiet,
    debug         => $debug,
...
for my $id ( @{ $pks->{ids} } ) {
    ...
    my $set = AIR2::PublicSrcResponseSet->new( $column_name => $id )
        ->load( speculative => 1 );
    if ( !$set or $set->not_found ) {
        warn
            "No PublicSrcResponseSet found where $column_name=$id. Skipping.\n";
        next;
    }
    if ( $set->is_public() ) {
        make_xml($set);
