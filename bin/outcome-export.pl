...
my ( $help, $debug, $sources, $user_id, $org_id, $prj_id, $inq_id,
    $start_date, $end_date, $format, $count );
GetOptions(
    'help'          => \$help,
    'debug'         => \$debug,
    'sources'       => \$sources,
...
if ($format ne 'csv' && $format ne 'json' && $format ne 'email') {
    pod2usage('Invalid format specified! Valid formats are (csv, json, email)');
}
...
if ($format eq 'csv') {
    ...
    if ($org_id || $prj_id || $inq_id || $start_date || $end_date || $sources) {
        $body = "Your PINfluence report is attached as a CSV file. It includes:\n";
    }
    if ($org_id) {
