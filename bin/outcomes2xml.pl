...
my $mod_since = 0;
my $debug     = 0;
my $base_dir  = AIR2::Config->get_search_xml->subdir('outcomes');
my $pk_filelist;
my $offset;
my $limit;
GetOptions(
...
outcomes2xml.pl - convert db records to XML for indexing

=head1 SYNOPSIS

 outcomes2xml.pl [opts] [src_idN .. src_idNN]
    --help
...
unless ($quiet) {
    AIR2::Utils::logger(
        "Determining which Outcome records to serialize...\n");
}
...
for my $out_id ( @{ $pks->{ids} } ) {

    my $out
        = AIR2::Outcome->new( out_id => $out_id )->load( speculative => 1 );
    if ( !$out or $out->not_found ) {
        warn "No Outcome found where out_id=$out_id. Skipping.\n";
        next;
    }
...
sub make_xml {
    my $out = shift;
    my $pk  = $out->out_uuid;
    $debug and warn "serializing Outcome object $pk";
    ...
    $out->db->get_write_handle->dbh->do(
        "delete from stale_record where str_type = 'O' and str_xid = ?",
        {}, $out->out_id );
}
